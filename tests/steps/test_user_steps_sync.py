...
def create_user_with_id_1(test_session):
    ...
    import asyncio
    asyncio.run(test_session.commit())
    asyncio.run(test_session.refresh(user))


@given(parsers.parse('there are no users with ID {user_id:d}'))
def no_user_with_id(test_session, user_id):
    """Ensure no user exists with given ID."""
    import asyncio
    result = asyncio.run(test_session.execute(select(User).where(User.id == user_id)))
    user = result.scalar_one_or_none()
    assert user is None


@given("there is a user with Telegram chat ID 111111111")
def create_user_with_telegram_id(test_session):
